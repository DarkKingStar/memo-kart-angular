...
export class PaymentService {
  ...
  updatePayment(reqData: any) {
    console.log('update payment reqData ', reqData.payment_id , reqData.order_id);
    const url = `${this.API_BASE_URL}/payments/?payment_id=${reqData.paymentId}&order_id=${reqData.orderId}`;
    const headers = new HttpHeaders({
      'Content-Type': 'application/json',
      Authorization: `Bearer ${localStorage.getItem('jwt')}`,
    });
